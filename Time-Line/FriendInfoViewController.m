...
-(void)createAddFriendBtn{
    ...
    addBtn.frame = CGRectMake( kScreen_Width -65,kScreen_Height -85, 45, 45);
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex;{
    if(buttonIndex == 0){//add friends
        ...
        UINavigationController *m_nav=(UINavigationController *) g_AppDelegate.window.rootViewController ;
        self.swiper = [[SloppySwiper alloc] initWithNavigationController:m_nav];
        m_nav.delegate = self.swiper;
        [m_nav pushViewController:searchVc animated:YES ];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.userNote.text = friend.note;
    return cell;
}
