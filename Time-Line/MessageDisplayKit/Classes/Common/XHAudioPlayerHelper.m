...
- (void)playAudioWithFileName:(NSString*)fileName {
    if (fileName.length > 0) {
        ...
        if (_playingFileName && [fileName isEqualToString:_playingFileName]) {//上次播放的录音
             ...
             */
            NSData *tmpData= [NSData dataWithContentsOfURL:[NSURL URLWithString:fileName]];
            AVAudioPlayer *pl = [[AVAudioPlayer alloc] initWithData:tmpData error:nil];
            if(!pl){
                pl = [[AVAudioPlayer alloc] initWithContentsOfURL:[NSURL URLWithString:fileName] error:nil];
            }
            //[pl duration];
            pl.delegate = self;
            [pl play];
