...
- (void)finishSendMessageWithBubbleMessageType:(XHBubbleMessageMediaType)mediaType xHMessage:(XHMessage *) msg idOfActive:(NSString *) eid {
    switch (mediaType) {
        case XHBubbleMessageMediaTypeText: {
            [self.messageInputView.inputTextView setText:nil];
            if ([[[UIDevice currentDevice] systemVersion] floatValue] >= 7.0) {
            ...
            [self netWorkSendxHMessage: msg idOfEvent:eid]; //发送信息到服务器上....
            break;
        }
        case XHBubbleMessageMediaTypePhoto: {
             [self netWorkSendxHMessage: msg idOfEvent:eid]; //发送信息到服务器上....
...
-(void)netWorkSendxHMessage:(XHMessage *) msg idOfEvent:(NSString *)eid{
    ASIFormDataRequest *uploadImageRequest;
    if( msg.photo || msg.voiceUrl|| msg.voicePath ){
        uploadImageRequest = [ASIFormDataRequest requestWithURL:[NSURL URLWithString:[Go2_UploadSocialFile stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]]];
        ...
        if (msg.voiceUrl || msg.voicePath) {
            [uploadImageRequest setPostValue:@"voice" forKey:@"type"];
            [uploadImageRequest setPostValue:@"3" forKey:@"msg"];
            ...
            NSLog(@"%@",[NSData dataWithContentsOfFile:msg.voicePath]);
            
            [uploadImageRequest addData:[NSData dataWithContentsOfFile:msg.voicePath] withFileName:[NSString stringWithFormat:@"%@.aac", tmpDate]  andContentType:@"audio/x-mei-aac" forKey:@"f1"];
        }else{
            [uploadImageRequest setPostValue:@"chat" forKey:@"type"];
            [uploadImageRequest setPostValue:eid forKey:@"eid"];
            NSData *data = UIImageJPEGRepresentation(msg.photo,0.5f);
        ...
        uploadImageRequest = [t_Network httpPostValue:@{@"method":@"sendChatMsg",@"msg":msg.text==nil?@"":msg.text,@"eid":eid}.mutableCopy
                                                  Url:Go2_socials
                                             Delegate:nil
                                                  Tag:Go2_SendMsg_tag];
    }
