...
-(void)backToEventView:(UIButton *) sender{
    switch (sender.tag) {
        case 1:{
            if (self.delegate && [self.delegate respondsToSelector:@selector(closeSegmentPageController:)]) {
                [self.delegate closeSegmentPageController:self];
            }
        }break;
        case 2:{
            if (self.delegate && [self.delegate respondsToSelector:@selector(openActiveSetingTableViewController:)]) {
                [self.delegate openActiveSetingTableViewController:self];
            }
        }break;
        default:
            break;
