...
- (void)layoutSubviews
{
    ...
    [self.subviews enumerateObjectsUsingBlock:^(UIButton *btn, NSUInteger idx, BOOL *stop) {
        ...
        CGFloat x = columnIndex * w;
        CGFloat y = rowIndex * h;
        btn.frame = CGRectMake(x, y, w, h);
    }];

    self.frame = CGRectMake(0, 0, kScreen_Width, totalRowCount * (h+SDPhotoGroupImageMargin));
}
...
{
    NSString *urlStr = [[self.photoItemArray[index] tremendous_pic] stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
