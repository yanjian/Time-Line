...
#define DATE_FORMAT_DAY_PORTIONS @[@0.4, @0.3, @0.33]
...
- (void)setSelected:(BOOL)selected animated:(BOOL)animated
{
    ...
    if (selected) {
        //        _label.font = [_textFont fontWithSize:_textFont.pointSize + 3];
...
- (void)drawRect:(CGRect)rect
{
    ...
    CGContextSaveGState(context);
    CGContextSetFillColorWithColor(context, [_bannerColor colorWithAlphaComponent:1.0f].CGColor);
    CGContextFillRect(context, bannerRect);
    CGContextRestoreGState(context);
    

        CGRect bannerRect1 = CGRectMake(105, (rect.size.height-_bannerHeight)/2+25, 4, 4);
        CGContextSaveGState(context);
        CGContextSetFillColorWithColor(context, [[UIColor whiteColor] colorWithAlphaComponent:1.0f].CGColor);
        CGContextFillRect(context, bannerRect1);
        CGContextRestoreGState(context);
        
        CGRect bannerRect2 = CGRectMake(105, (rect.size.height-_bannerHeight)/2+15, 4, 4);
        CGContextSaveGState(context);
        CGContextSetFillColorWithColor(context, [[UIColor whiteColor] colorWithAlphaComponent:1.0f].CGColor);
        CGContextFillRect(context, bannerRect2);
        CGContextRestoreGState(context);
...
- (void)setDate:(NSString*)date
{
    ...
    selectedHour=[[datearray objectAtIndex:0] integerValue];
    selectedMinute=[[datearray objectAtIndex:1] integerValue];
    [self refreshTables];
}
...
-(void)setHourAtty:(NSArray *)hourAtty{
    _hourAtty=hourAtty;
    [self refreshTables];
...
- (void)setDefaults
{
    ...
    selectedMonth = 1;
    selectedDay = 1;
    _pickerColor = RGBA(168, 166, 72, 1);
    _separatorLineColor = RGBA(140, 158, 159, 1);
    _bgColor = RGBA(168, 183, 185, 1);
    _font = [UIFont systemFontOfSize:11];
    _cellHeight = self.frame.size.height/3;
    _dateFormat = JCDateFormatFull;
    _dayArray=[NSArray arrayWithObjects:@"1",@"2",nil];
    _hourAtty=[NSArray arrayWithObjects:@"1",@"2",nil];
    _minArray=[NSArray arrayWithObjects:@"1",@"2",nil];
...
{
    if (_dateFormat==JCDateFormatClock) {
        if (self.delegate && [self.delegate respondsToSelector:@selector(datePicker:dateDidChange:)]) {
            ...
            if (selectedMinute<10) {
                [self.delegate datePicker:self dateDidChange:[NSString stringWithFormat:@"%ld:0%ld",(long)selectedHour,(long)selectedMinute]];
            }else{
                [self.delegate datePicker:self dateDidChange:[NSString stringWithFormat:@"%ld:%ld",(long)selectedHour,(long)selectedMinute]];
            }
            [self refreshTable:[componentTables objectForKey:[NSNumber numberWithInt:SECOND_TAG]]];
            NSLog(@"%@",[NSString stringWithFormat:@"%ld:%ld",(long)selectedHour,(long)selectedMinute]);
        }
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(datePicker:dateDidChange:)]) {
            ...
            [self.delegate datePicker:self dateDidChange:[NSString stringWithFormat:@"%ld.%ld",(long)selectedMonth-1,(long)selectedDay-1]];
            [self refreshTable:[componentTables objectForKey:[NSNumber numberWithInt:SECOND_TAG]]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    switch (tableView.tag) {
        ...
        case MINUTE_TAG: {
            if (indexPath.row<10) {
                cell.label.text = [NSString stringWithFormat:@"0%d",indexPath.row];
            }else{
                cell.label.text = [NSString stringWithFormat:@"%d",indexPath.row];
            }
            break;
