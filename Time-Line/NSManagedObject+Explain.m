...
+ (void)asyncQueue:(BOOL)async actions:(void (^)(void))actions
{
    ...
    {
        myCustomQueue = dispatch_queue_create("com.zhilifang.coredata", DISPATCH_QUEUE_SERIAL); //生成一个串行队列
        
        CFStringRef specificValue = CFSTR("com.zhilifang.coredata");
    ...
    if (retrievedValue && [retrievedValue isEqualToString:@"com.zhilifang.coredata"]) {
        actions ? actions() : nil;
    }else{
        if(async){
